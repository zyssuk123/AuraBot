...
BLOB_TOKEN = (os.getenv("BLOB_READ_WRITE_TOKEN") or "").strip().strip('"').strip("'")
...
def upload_photo(person_name: str, photo_index: int, image_bytes: bytes) -> str:
    ...
    print(f"[Blob] Photo uploadee : {url}")
    return url
...
def download_gallery_to_tempdir() -> str:
    ...
    blobs = list_photos()
    if not blobs:
        print("[Blob] Galerie vide - aucune photo a telecharger.")
    ...
    print(f"[Blob] Galerie prete dans {tmp_dir}")
    return tmp_dir
...
def delete_person(person_name: str) -> int:
    ...
    if response.status_code == 200:
        print(f"[Blob] {len(urls)} photos supprimees pour {person_name}")
        return len(urls)
    else:
