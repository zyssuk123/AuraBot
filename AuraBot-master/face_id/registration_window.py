...
try:
    from face_id.cloud_gallery import upload_photo
except ImportError:
...
class FaceRegistrationWindow(ctk.CTkToplevel):
    ...
    def __init__(self, master, gallery_path, engine, audio_mgr, on_complete):
        ...
        self.gallery_path = os.path.abspath(gallery_path)
    ...
    def _save_local_face(self, person_name, roi, photo_index):
        safe_name = person_name.replace(" ", "_").capitalize()
        person_dir = os.path.join(self.gallery_path, safe_name)
        os.makedirs(person_dir, exist_ok=True)
        local_path = os.path.join(person_dir, f"{safe_name}_{photo_index}.jpg")
        ok, encoded = cv2.imencode(".jpg", roi, [cv2.IMWRITE_JPEG_QUALITY, 90])
        if not ok:
            raise RuntimeError(f"Impossible d'écrire {local_path}")
        encoded.tofile(local_path)
        return local_path
    ...
    def ask_name_vocal(self):
        ...
        self.audio.parler(self.audio._text("ask_name"))
    ...
    def listen_name(self):
        ...
        def error(msg):
            ...
            self.audio.parler(self.audio._text("retry_name"))
    ...
    def start_scan_vocal(self):
        ...
        self.audio.parler(self.audio._text("scan_start"))
        ...
        if ret:
            ...
            if self.is_scanning and len(faces) > 0:
                ...
                if t - self.last_capture_time > 0.2:
                    ...
                    def _persist_face(n=name, i=idx, roi_copy=roi):
                        try:
                            self._save_local_face(n, roi_copy, i)
                            print(f"[Local] ✅ Photo sauvegardée pour {n}: {i}")
                        except Exception as e:
                            print(f"[Local] Erreur sauvegarde photo {i}: {e}")

                        try:
                            upload_photo(n, i, image_bytes)
                            print(f"[Blob] ✅ Upload photo {i} pour {n}")
                        except Exception as e:
                            print(f"[Blob] Erreur upload photo {i}: {e}")

                    persist_thread = threading.Thread(target=_persist_face, daemon=True)
                    self._persist_threads.append(persist_thread)
                    persist_thread.start()
                    
                    self.progress.set(self.photos_taken / self.max_photos)
                    self.lbl_info.configure(
                        text=f"💾 Sauvegarde {self.photos_taken}/{self.max_photos}...",
                        text_color="#1E90FF"
                    )
                    
                    if self.photos_taken >= self.max_photos:
                        self.is_scanning = False
                        self.lbl_info.configure(text="✅ PROFIL ENREGISTRÉ", text_color="#2ECC71")
    ...
    def finish(self):
        for persist_thread in self._persist_threads:
